...
- HTML 문서는 휴대폰 가로형 화면을 TV로 미러링해 발표하는 용도를 최우선으로 취급한다.
- 내용 페이지는 항상 16:9 안에 들어와야 하며, 사진이 레이아웃을 밀어내거나 화면비율을 깨면 안 된다.
- 사진 영역은 7:3 좌우 그리드의 왼쪽에 고정하고 `object-fit: contain`, `max-width`, `max-height`, `overflow: hidden` 규칙을 유지한다.
- 휴대폰 가로형에서는 글자 크기를 줄이더라도 사진과 설명이 한 화면 안에 보이도록 한다.
