...
class PhotoPurger:
    ...
    def purge_photo_data(abs_path: str, point_id: str = None, keep_original: bool = False):
        ...
        try:
            ...
            error_msg = f"물리적 원본/파생 파일 삭제 중 오류 발생: {str(e)}"
            print(f"   ❌ [1/3] {error_msg}")
        ...
        if point_id:
            try:
                ...
                error_msg = f"Qdrant DB 포인트 삭제 통신 오류: {str(e)}"
                print(f"   ❌ [2/3] {error_msg}")
                errors.append(error_msg)
        ...
        if point_id:
            cache_err = PhotoPurger._remove_from_static_caches(point_id)
            if cache_err:
    ...
    def _remove_from_static_caches(point_id: str):
        ...
        if os.path.exists(timeline_path):
            try:
                ...
                err_msg = f"timeline_cache.json 업데이트 실패: {str(e)}"
                print(f"   ⚠️ {err_msg}")
