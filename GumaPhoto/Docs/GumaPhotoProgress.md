...
## 📌 [21단계] 클러스터 3D 데칼 프로젝션(Decal Projection) 렌더링 도입 테스트 (진행중 🚀)
*   기존 클러스터가 카메라를 계속 정면으로 응시하는 2D 빌보드(Billboard) 방식이라 3D 구체의 공간감을 해친다는 피드백을 수용하여, 마커를 지표면에 붙여버리는 텍스쳐 프로젝션 방식을 시도했습니다.
*   Cesium의 `CallbackProperty`를 이용해 현재 카메라의 고도(Distance)를 실시간 연산하고, 캔버스를 `EllipseGraphics` (지표면 데칼 모양)의 머티리얼로 매핑하여 카메라 줌에 따라 물리적 반경(Radius)이 능동적으로 변형되도록 구현했습니다.
