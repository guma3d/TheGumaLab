...
## 📌 [23단계] 클러스터 돔 롤백 및 솔리드 코어(Solid Core) 아우라 마커 디자인으로 선회 (완료 🎯 - 오늘 진행)
*   반구형(Dome) 모델링 시야에 대한 호불호 및 피드백을 수용하여, 즉각 기존 `BOTTOM` (세로 앵커) 기반의 부양 스탠딩 마커 구조로 원상 복구(Rollback)했습니다. 글씨 위치 또한 코어 정중앙으로 재정렬 완료.
*   하지만 단순한 단색 원형이 아니라 **가장자리 페더링(Feathering)** 기법을 도입하여 세련미를 살렸습니다. 
*   원의 중심부터 **80% 지점까지는 0.95의 거의 불투명한 솔리드 컬러(Solid Rose)**로 채워 글씨와 덩어리감을 선명히 잡고, **80% ~ 100% 지점(가장자리 끝자락) 구간은 투명도(Alpha)가 0.0 으로 서서히 소멸하는 방사형 그라데이션**을 적용하여 고급스러운 빛 입자(Aura) 느낌을 연출했습니다.
