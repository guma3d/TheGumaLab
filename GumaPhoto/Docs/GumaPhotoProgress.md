...
## 📌 [23단계] 클러스터 돔 롤백 및 중공형 테두리(Hollow Ring) 아우라 마커 디자인으로 선회 (완료 🎯 - 오늘 진행)
*   반구형(Dome) 모델링 시야에 대한 호불호 및 피드백을 수용하여, 즉각 기존 `BOTTOM` (세로 앵커) 기반의 부양 스탠딩 마커 구조로 원상 복구(Rollback)했습니다. 글씨 위치 또한 코어 정중앙으로 재정렬 완료.
*   단순한 단색 원형이 아니라 **중앙이 비어있는 테두리 링(Hollow Ring) 페더링** 기법을 도입하여 세련미를 살렸습니다. 
*   원의 중심부터 **20% 지점까지는 Alpha=0.0 (완전 투명)** 상태를 유지하여 뒷배경 지도가 맑게 비치게 만들고, **20% ~ 100% 지점(가장자리 끝자락) 구간은 Alpha=0.95(거의 불투명) 로 서서히 색이 짙어지는 방사형 그라데이션**을 적용하여 네온사인처럼 은은하게 빛나는 외곽선 느낌을 연출했습니다.
