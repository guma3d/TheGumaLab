...
*   지표면에 텍스쳐를 투영시키는 데칼(Decal) 프로젝션 방식을 테스트하였으나, 지평선 부근에서의 글씨 왜곡과 투영(Clip) 이슈를 우려하여 즉시 3D 입체 투명 구체(Sphere) 방식으로 롤백 후 재구현했습니다.
*   클러스터 위치에 `EllipsoidGraphics`를 생성하고 땅 파고들기(`CLAMP_TO_GROUND`) 옵션을 주어 자연스러운 빛 반사 반구(Dome) 형태를 띄도록 수정했습니다. 숫자 텍스트는 3D 오브젝트나 지형에 파묻히지 않도록 `disableDepthTestDistance`를 `무한대`로 주고 화면 최상단 순정 `Label`로 띄워 완벽한 가시성을 확보했습니다.
