...
📦 <body> (전체 화면의 컨테이너)
 ...
 ┃
 ┣━ 🛠️ [2] <div id="selection-fab"> (화면 하단 플로팅 액션 바 / z-index: 200)
 ┃   ┗━ 모바일에서 사진을 꾹 누르거나 여러 장 선택했을 때, 숨어있다가 부드럽게 올라오는 하단 다운로드/취소 바
 ┃
 ┗━ ⬛ [3] 팝업 모달 레이어 (클릭 시 화면 전체를 덮는 컴포넌트 / z-index: 1000+)
     ┣━ 📸 <div id="photo-modal"> (사진을 확대해서 보고 개별 코멘트 제출 및 삭제(Trash) 명령을 내리는 창)
