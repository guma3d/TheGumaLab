...
*   **동작 방식:** 폴더 정리가 끝난 직후 **Redis/Celery 체인**에 의해 바로 배턴 터치를 받아 분리 가동되는 심층 스캐너입니다.
    ...
    4.  **초경량 패칭 스위치 (Modular Loading Toggle):** 모델 전체를 메모리에 올리는 오버헤드를 막기 위해 `--update-location` CLI 플래그 등 5가지 스위치(`skip_face`, `run_metadata_geo` 등)를 지원합니다. 무거운 모델 로드를 생략하고 초고속으로 Qdrant Payload의 장소/날짜만 `set_payload` 하는 지능형 우회 메커니즘을 내장하고 있습니다.
...
1. **실시간 마스터 사전 맵핑 (Gemini Entity Resolution)**
   - 백엔드의 `/app/data/available_tags.json` 마스터 사전을 실시간으로 읽어와 Gemini 3.1 Flash 모델의 통찰력에 그대로 던집니다.
   - **[오타 및 다국어 보정 알고리즘]**: 사용자가 "하와이", "글랜데일" 이라고 엉망으로 쳐도, Gemini가 알아서 찰떡같이 사전에 존재하는 `["미국 Hawaii"]`, `["미국 Glendale"]` 형태의 JSON 배열 리스트를 추출해 줍니다. 과거처럼 `difflib`이나 불필요한 행정구역 문자열(`특별시` 등) 슬라이싱 등의 조잡한 코드가 일절 필요 없습니다.
   - **[무의미한 검색어(Filler Words) 파쇄]**: `보여줘`, `사진 찾아봐` 같은 대화형 빈말에 SigLIP이 반응하는 것을 막기 위해, Gemini가 시각적 단어가 아예 없다고 판단하면 `EMPTY`를 반환하도록 지시하여 무거운 벡터 검색을 우회시키는 안전판이 존재합니다.
2. **철통 방어 1차 수문장 (Must/Should Condition Filtering)**: 
   - Gemini가 찾아낸 장소들과 인물 식별자를 가져와 Qdrant에 강력한 교집합(MUST) 로직을 구사합니다.
   - **[Qdrant TEXT Index 버그 방어]**: `location` 필드가 토큰 인덱스라 `MatchAny` 연산이 무시되는 Qdrant 내부 버그를 피하고자, 즉각 `Filter(should=[MatchText...])` 식의 다중 OR 쿼리로 동적 치환하여 검색 엔진 누락을 원천 봉쇄합니다.
3. **무한 탐색 벡터 검색 (가중치 70%)**: 필터를 통과한 영문 장면 후보들을 SigLIP 768D 공간에서 탐색하여 가장 유사한 분위기/시각적 사진들을 나열합니다.
