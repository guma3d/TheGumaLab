    ...
    if gemini_client and not req.is_load_more:
        try:
            ...
            prompt = (
                ...
                "2) 사용자 검색어 중 장소/위치/국가명(예: 성남, 부산, 하와이 등)이 포함되어 있다면, 다음 <보유 장소 목록> 중에서 유추하여 텍스트 원본 그대로 'location'에 문자열로 반환하세요.\n"
                f"   <보유 장소 목록> : {existing_locations}\n"
                "   (핵심 규칙: '하와잌'처럼 오타가 나거나 부분이름('성남')을 검색해도 목록 내의 '미국-하와이', '경기도-성남시' 등으로 똑똑하게 유추 매칭해야 합니다. 단, '알래스카'처럼 목록의 위치들과 아예 무관한 장소라면 절대 억지로 맞추지 말고 반드시 빈 문자열('')을 넣으세요!)\n"
                ...
                "예시: {\"people\": [\"송이\"], \"location\": \"미국-하와이\", \"objects\": [\"dog\", \"blue shirt\"], \"scene\": \"a young girl playing at a wedding ceremony\"}\n"
...
async def delete_photo(req: DeleteRequest):
    ...
    try:
        ...
        try: conn.execute("DELETE FROM processed_files WHERE filepath=?", (abs_path,))
        except: pass
...
async def submit_feedback_v2(req: FeedbackV2Request):
    ...
    # 2. Date 피드백인 경우
    elif fb_type == "time_loc" and ("Date" in req.issue_type or "날짜" in req.issue_type) and gemini_client:
        prefix = "DATE|"
        try:
            prompt_date = (
            ...
            response_date = gemini_client.models.generate_content(
                model='gemini-3.1-flash-lite-preview',
    ...
    try:
        ...
        conn.execute(
            ...
            (str(req.point_id), fb_type, db_correct_value, tp_json)
        )
        conn.commit()
        conn.close()
