    ...
    if gemini_client and not req.is_load_more:
        try:
            ...
            prompt = (
                ...
                "2) 사용자 검색어 중 장소/위치/국가명(예: 성남, 부산, 하와이 등)이 포함되어 있다면, 의미를 스스로 유추하여 다음 <보유 장소 목록> 중 가장 일치하는 텍스트 원본 그대로를 'location'에 문자열로 저장하세요.\n"
                f"   <보유 장소 목록> : {existing_locations}\n"
                "   (핵심 규칙: '성남'을 검색하면 '경기도-성남시', '일본 오사카'는 '일본-오사카시' 등과 같이 무조건 보유 장소 목록에 있는 한글 명칭 철자 하나 틀리지 않고 100% 동일하게 치환해야 합니다. 부분 일치나 포함관계인 장소(예: '경기도' 검색 시 '경기도-성남시')도 매칭 대상입니다. 정합되는 게 없으면 빈 문자열을 넣으세요.)\n"
