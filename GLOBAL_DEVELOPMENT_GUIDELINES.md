...
2. **버전 관리 (AI 자동화)**: 변경 사항을 GitHub 서버로 Push (Commit).
3. **서버 반영 (AI 자동화)**: HomeServer(메인 컴퓨터)에서 SSH 접속 또는 자동화 스크립트를 통해 `git pull` 수행.
4. **서비스 재가동 (AI 자동화)**: 필요 시 Docker 컨테이너 재시작 등 배포 파이프라인 가동.

⚠️ **개발 원칙 합의 사항**
- 코드는 **오직 GitHub 서버**에만 푸시(Commit)합니다.
- 홈서버(HomeServer)의 구동 중인 로컬 코드를 실시간으로 직접 수정하지 않습니다. (핫픽스 제외)
- **AI 어시스턴트(Antigravity) 행동 지침:** 코드 수정이 완료되면 사용자의 별도 지시가 없더라도 자동으로 `Sync and Push` 작업을 진행하고, `ssh HomeServer`를 통해 `pull_update.bat` 실행 및 관련 Docker 워커들의 재시작까지 One-Stop으로 처리해 주어야 합니다.
