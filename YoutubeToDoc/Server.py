...
def rerank_with_llm(query: str, results: list, top_k: int = 10,llm_model="gemini-2.5-flash",):
...
def transcribe_audio(audio_path: str, srt_path: str):
    ...
    prompts = get_prompts()
    whisper_model = prompts.get("whisper_model", "gemini-2.5-flash") # 속도/비용 면에서 flash 권장
    ...
    try:
        ...
        if gemini_client:
            try:
                ...
                query_expansion_model = prompts.get("query_expansion_model", "gemini-2.5-flash")
                if query_expansion_model.startswith("gpt-"):
                     query_expansion_model = "gemini-2.5-flash"
        ...
        if results and gemini_client and enable_search_answer == 1:
            try:
                ...
                answer_model = prompts.get("answer_model", "gemini-2.5-flash")
...
def admin_save():
    ...
    translation_model = data.get('translation_model', 'gemini-2.5-flash')
    ...
    query_expansion_model = data.get('query_expansion_model', 'gemini-2.5-flash')
    answer_system_prompt = data.get('answer_system_prompt', '')
    answer_model = data.get('answer_model', 'gemini-2.5-flash')
