...
- **안전필터(Safety Settings) 우회**: 게임 영상이나 폭력적인 내용 등으로 인해 번역이 차단(Block)되는 것을 방지하기 위해 `HARM_CATEGORY_HARASSMENT` 등 4개 위험 카테고리 필터를 `BLOCK_NONE`으로 명시적 비활성화하여 API 호출을 수행합니다.
...
- **과거 문서 재생성 스크립트 (`regen.py`)**: 현재까지 쌓아놓은 로컬 데이터를 전부 스캔해서 최신기능(예: 태그 삽입 등) 기준으로 기존 사용 문서 HTML포맷 전체를 한 번에 덧씌워 파싱 재생성시킬 수 있습니다.
- **오류 문서 수동 재작업 시스템 (`/retranslate`)**: 
  - 특정 문서 번역이 고장나거나 누락되었을 때 백에서 강제로 재실행하기 위한 엔드포인트입니다.
  - 클린업 과정에서 삭제된 원본 데이터(srt, json 등) 대신, 살아남은 상세 HTML(`[Title]-detail.html`) 구조를 뜯어 파싱(BeautifulSoup 4)하여 세그먼트 데이터를 역추출(Reverse Extract)합니다.
  - 새롭게 파싱한 데이터를 기반으로 안전필터가 해제된 Gemini API를 돌려 텍스트를 재번역하고, 상세 가이드 문서 및 요약 문서를 다시(Overwrite) 생성해 무중단 복원을 지원합니다.
