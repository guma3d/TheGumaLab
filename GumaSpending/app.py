...
def _build_prompt(data: dict, is_current: bool) -> str:
    ...
    common_sections = [
        "",
        "아래 3개 섹션 구조로 한국어 분석을 작성해주세요. 각 섹션은 `## 제목` 마크다운 헤더로 시작합니다.",
        "",
        "## 분석 요약",
        "- 이달 지출의 종합 의견을 작성하세요.",
    ...
    if is_current:
        ...
        lines += exclusion_rules + [
            ...
            "",
            "추가 요청 (현재 진행 중인 달이므로):",
            "- '분석 요약'에 남은 기간 소비 조언과 이달 말 예상 총 지출도 포함해주세요.",
            "- '일반적인 지출'에서 남은 일수 대비 일평균을 고려한 조언도 추가해주세요.",
        ]
    else:
    ...
    lines += [
        "",
        "작성 규칙:",
        "- 각 섹션 제목은 위에 명시된 3개(분석 요약 / 주요 특이점 / 일반적인 지출)를 그대로 사용하세요.",
